...
First, we create `csr.conf` with the following content[3]:
```
[ req ]
default_bits = 2048
prompt = no
...
Then, edit `csr.conf` and change values marked with angle brackets ( `<xxx>` ).
Note that `<MASTER_CLUSTER_IP>` is the first IP of the service CIDR that is specified as the `--service-cluster-ip-range` argument for API server.

Next, we must fetch CA key and certificate from minikube VM.
...
$ openssl x509 -req -in server.csr -CA ca.crt -CAkey ca.key -CAcreateserial \
    -out client.crt -days 3650 -extensions v3_ext -extfile csr.conf
```
